...
async def test_scheduler():
	...
	async def run_prompt(prompt_id, prompt_text):
		# Submit to the engine queue
		results_generator = engine.generate(prompt_text, sampling_params, request_id=str(prompt_id))
	...
	# 2. Submit all prompts simultaneously
	...
	for i, t in enumerate(sorted_tracker):
		...
		print(f"Order: {i+1} | Original ID: {t['id']} | Char Length: {t['length']:<3} | Duration: {duration:.4f}s")
